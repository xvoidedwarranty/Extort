...
local SaveManager = {} do
	SaveManager.Folder = getgenv().settings_folder or "ExtortSettings"
	SaveManager.Ignore = {}
	SaveManager.Parser = {
		Toggle = {
	...
	function SaveManager:BuildConfigSection(tab,disablecloudconfigs)
		...
		if not disablecloudconfigs then
			...
			local BrowserLIB = loadstring(game:HttpGet("https://raw.githubusercontent.com/laagginq/Extort/main/browserv2.lua"))()
			local Configs = game:GetService("HttpService"):JSONDecode(httprequest({Url = 'https://raw.githubusercontent.com/laagginq/Extort/main/configs.json'}).Body)['Configs']
